...
The packaged application includes Electron and its Node.js runtime; users do **not**
need to install Node.js separately.

- **Windows:** 64-bit Windows 10 or later. NSIS installer (`x64`).
- **macOS:** the release pipeline currently builds Intel (`x64`) DMGs. Apple Silicon
  requires Rosetta for these artifacts; native ARM64 releases are not currently
  built. OS compatibility follows the bundled Electron release, not the original
  Mechvibes minimums.
- **Linux:** `x64` deb, snap and AppImage packages. Global keyboard capture relies
  on native X11 libraries; Wayland sessions may restrict global input capture.
  Test on the target distribution rather than assuming kernel version alone
  guarantees compatibility.

Global input capture and output-device behavior must be checked on the target OS.
On macOS, grant Accessibility/Input Monitoring permissions when requested.
...
Use **Node.js 22.22.0** from `.nvmrc`, or Node.js 24. CI verifies both runtime lines.
...
| `npm run verify` | Main build, syntax/soundpack checks, both test suites, all type checks, renderer build + asset checks, website build |
| `npm run diagnose -- [installation-directory]` | Check working-tree/installed renderer assets and preload paths |
| `npm run benchmark:audio` | Synthetic scheduling-budget regression check (not measured hardware latency) |
