...
export class WebAudioEngine {
  ...
  async loadManifest(manifest: AudioManifest): Promise<void> {
    ...
    this.cache = new SampleCache({
      context: this.context as unknown as AudioContext,
  ...
  applyMasterGain(): void {
    ...
    this.masterGainNode.gain.setValueAtTime(
      clamp(this.masterGain * packGain, 0, 2),
  ...
  play(event: PlaybackEvent): Promise<boolean> {
    ...
    if (cached) {
      return Promise.resolve(this.playBuffer(cached as unknown as AudioBuffer, sample, layer, event));
    }
    this.metrics.cacheMisses += 1;
    return this.cache
      .load(sample.source)
      .then((buffer) => this.playBuffer(buffer as unknown as AudioBuffer, sample, layer, event));
  }
  ...
  playBuffer(
    ...
    (this.voicePool as VoicePool).reserve({
      source,
