...
declare global {
  namespace Electron {
  interface App {
    /** Set by Mechvibes to tell a real quit apart from a window close. */
    isQuiting?: boolean;
    }
  }
}
...
// electron-log v5. Only the local file and console transports are used; the
// former remote transport that uploaded diagnostics to an external server has
// been removed.
...
(global as any).current_pack_store_id = current_pack_store_id;
...
function validateSoundpackCandidate(candidatePath: string) {
  ...
  try {
    listReferencedSoundFiles(config).forEach((reference: string) => GetSoundpackFile(candidatePath, reference));
...
function customPackPath(packId: string) {
...
function handleProtocolUrl(url: string) {
  ...
  try {
    ...
    if (command && Object.prototype.hasOwnProperty.call(protocolCommands, command)) {
      (protocolCommands as Record<string, (...args: string[]) => void>)[command](...parts);
    }
  } catch (error) {
...
if (!gotTheLock) {
  ...
  app.on('second-instance', (event, commandLine, workingDirectory) => {
    ...
    if (win) {
      if (process.platform === 'darwin') {
        ...
        handleProtocolUrl(protocolUrl as string);
      }
      if (win.isMinimized()) {
        win.restore();
      }
  ...
  app.on('ready', () => {
    ...
    const setInputHookEnabled = (enabled: boolean) => {
      try {
    ...
    ipcMain.handle('open-external', async (event, value) => {
      if (![win, installer, editor_window].some((window) => isWindowEvent(event, window))) return false;
    ...
    ipcMain.on('electron-log', (event, message, level) => {
      if (![win, installer, editor_window].some((window) => isWindowEvent(event, window))) return;
    ...
    if(!storage_prompted.is_enabled){
      ...
      if(fs.existsSync(old_custom_dir)){
        ...
        if (response === 0) {
          ...
          oldCustomFiles.forEach((file: string) => {
...
let editor_window = null as unknown as import('electron').BrowserWindow;
