...
export type RemapPlatform = 'standard' | 'linux' | 'darwin' | 'win32';
export type RemapDefines = Record<string, unknown>;

/**
 * Remaps soundpack key definitions between the platform-neutral "standard"
 * layout and a concrete platform layout (and back).
 */
function remapper(
  from: RemapPlatform = 'standard',
  to: RemapPlatform = 'darwin',
  defines: RemapDefines = {},
): RemapDefines {
  if (from === 'standard') {
    // from standard to linux, darwin, win32
    switch (to) {
      case 'linux':
        break;
      case 'darwin': {
