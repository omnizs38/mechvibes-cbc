...
interface BrowserWindowOptionsCarrier {
  browserWindowOptions?: { name?: unknown } | undefined;
}
...
export function resolveLogSenderName(event: Pick<IpcMainEvent, 'sender'>): string {
  const window_options = (event.sender as unknown as BrowserWindowOptionsCarrier)
