...
/** A key slot is a keycode, `0` for a gap, or a group of alternative keycodes. */
export type KeySlot = number | number[];
export type LayoutRow = KeySlot[];
export type LayoutZone = 'main' | 'edit' | 'numpad';
export type Layout = Record<LayoutZone, LayoutRow[]>;
...
export function remap(to: NodeJS.Platform | string = 'win32'): Layout {
...
export type KeySizes = Record<number, string>;
...
const win32 = remap('win32');
const darwin = remap('darwin');
const linux = standard;

export { standard, win32, darwin, linux, sizes };
