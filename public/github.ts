...
export function safeReleaseUrl(value: unknown, asset = false): string | null {
...
export function selectReleases(values: unknown): { latest: Release | null; preview: Release | null } {
  if (!Array.isArray(values)) throw new Error('Unexpected GitHub release response.');
...
function readCache(): ProjectData | null {
  try {
    ...
    if ((data.latest !== null && !latest) || (data.preview !== null && !preview)) return null;
...
async function fetchProjectData(force: boolean): Promise<ProjectData> {
  const cache = readCache();
  if (!force && cache && Date.now() - cache.fetchedAt < FRESH_MS) return cache;
  ...
  try {
    ...
    const data: ProjectData = { ...selectReleases(await response.json()), fetchedAt: Date.now(), source: 'network' };
    try {
    ...
    if (cache) return { ...cache, source: 'stale' };
    throw error;
  } finally {
    clearTimeout(timer);
  }
}
export function platformFromHints(platform: string, userAgent: string, maxTouchPoints = 0): Platform {
  if (/android|iphone|ipad|ipod/i.test(userAgent) || (/mac/i.test(platform) && maxTouchPoints > 1)) return 'unknown';
...
export function pickInstaller(release: Release | null, platform: Platform): ReleaseAsset | null {
