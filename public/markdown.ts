 ...
 */
const HEADING_OFFSET: number = 3;

const FENCE_RE: RegExp = /^ {0,3}(`{3,}|~{3,})\s*([^`\s]*)\s*$/;
const HEADING_RE: RegExp = /^ {0,3}(#{1,6})\s+(.*?)\s*#*\s*$/;
const UL_ITEM_RE: RegExp = /^ {0,3}[-*]\s+(.*)$/;
const OL_ITEM_RE: RegExp = /^ {0,3}(\d{1,9})[.)]\s+(.*)$/;
const BLANK_RE: RegExp = /^\s*$/;
 ...
 */
export function truncateMarkdown(source: string, limit: number): Truncated {
  ...
  if (!kept.length) {
    ...
    const space: number = slice.lastIndexOf(' ');
...
export function renderMarkdown(source: string): DocumentFragment {
  ...
  while (i < lines.length) {
    ...
    const fence: RegExpExecArray | null = FENCE_RE.exec(line);
    if (fence) {
...
function appendCodeBlock(
  ...
  const marker: string = fence[0] ?? '`';
  const closing: RegExp = new RegExp(`^ {0,3}\\${marker}{${fence.length},}\\s*$`);
  const body: string[] = [];
...
function appendHeading(parent: DocumentFragment, level: number, text: string): void {
  const tag: string = `h${Math.min(level + HEADING_OFFSET, 6)}`;
  const heading: HTMLElement = document.createElement(tag);
  heading.className = `md-h${level}`;
  renderInline(text, heading);
  parent.appendChild(heading);
}

function appendList(parent: DocumentFragment, lines: string[], start: number): number {
  const first: string = lines[start] ?? '';
  const ordered: boolean = OL_ITEM_RE.test(first);
  const list: HTMLElement = document.createElement(ordered ? 'ol' : 'ul');

  if (ordered) {
    const startAt: number = Number(OL_ITEM_RE.exec(first)?.[1] ?? '1');
    if (startAt !== 1) (list as HTMLOListElement).start = startAt;
  }
  ...
  for (; i < lines.length; i += 1) {
    ...
    const ol: RegExpExecArray | null = OL_ITEM_RE.exec(line);
    const ul: RegExpExecArray | null = UL_ITEM_RE.exec(line);
...
function appendParagraph(parent: DocumentFragment, lines: string[], start: number): number {
  const block: string[] = [];
  let i: number = start;
...
const CODE_RE: RegExp = /^(`+)([\s\S]*?)\1(?!`)/;
const LINK_RE: RegExp = /^\[([^\]\n]*)\]\(\s*([^\s()]*)(?:\s+"[^"\n]*")?\s*\)/;
const AUTOLINK_RE: RegExp = /^https?:\/\/[^\s<>"'`]+/;
const STRONG_RE: RegExp = /^(\*\*|__)(?=\S)([\s\S]*?\S)\1/;
const EM_RE: RegExp = /^([*_])(?=[^\s*_])([\s\S]*?[^\s*_])\1(?!\1)/;
 ...
 * inside the next.
 */
function renderInline(text: string, parent: Node, inLink: boolean = false): void {
  let buffer: string = '';
  let i: number = 0;
  ...
  while (i < text.length) {
    ...
    const code: RegExpExecArray | null = char === '`' ? CODE_RE.exec(rest) : null;
    if (code) {
      flush();
    ...
    const link: RegExpExecArray | null =
      char === '[' && !inLink ? LINK_RE.exec(rest) : null;
    if (link) {
      flush();
    ...
    const auto: RegExpExecArray | null =
      char === 'h' && !inLink ? AUTOLINK_RE.exec(rest) : null;
    if (auto) {
      const url: string = trimUrlTail(auto[0]);
      const anchor: HTMLAnchorElement | null = safeAnchor(url, url);
      if (anchor) {
        flush();
    ...
    const strong: RegExpExecArray | null =
      char === '*' || char === '_' ? STRONG_RE.exec(rest) : null;
    if (strong) {
      flush();
    ...
    const emAllowed: boolean = char === '*' || !/\w/.test(text[i - 1] ?? '');
    const em: RegExpExecArray | null =
      emAllowed && (char === '*' || char === '_') ? EM_RE.exec(rest) : null;
    if (em) {
      flush();
 ...
 */
function appendLink(parent: Node, label: string, href: string, raw: string): void {
  const anchor: HTMLAnchorElement | null = safeAnchor(href, label || href);
  if (!anchor) {
 ...
 */
function safeAnchor(href: string, label: string): HTMLAnchorElement | null {
  let parsed: URL;
  try {
    parsed = new URL(href);
  } catch {
...
function trimUrlTail(url: string): string {
  ...
  while (end > 0) {
    const char: string = url[end - 1] ?? '';
    if ('.,;:!?'.includes(char)) {
      end -= 1;
      continue;
    }
    if (char === ')') {
      const slice: string = url.slice(0, end);
      const opens: number = (slice.match(/\(/g) ?? []).length;
      const closes: number = (slice.match(/\)/g) ?? []).length;
      if (closes > opens) {
        end -= 1;
        continue;
      }
