/** An opt-in, synthesized sound toy. No global key capture, microphone or persistence. */
type Tone = 'soft' | 'crisp' | 'deep';
export function setupDemo(): void {
  const input = document.querySelector<HTMLTextAreaElement>('#demoText');
  const keys = document.querySelector<HTMLElement>('#previewKeys');
  const toggle = document.querySelector<HTMLButtonElement>('#demoToggle');
  const status = document.querySelector<HTMLElement>('#demoStatus');
  const volume = document.querySelector<HTMLInputElement>('#demoVolume');
  if (!input || !keys || !toggle || !status || !volume) return;
  ...
  async function sound(code: string) {
    const key = keys!.querySelector<HTMLElement>(`[data-code="${code}"]`);
    if (key) {
  ...
  toggle.addEventListener('click', () => {
    void (async () => {
      try {
        ...
        document.querySelector<HTMLElement>('#experience')!.dataset.enabled = String(enabled);
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-tone]').forEach((button) =>
    button.addEventListener('click', () => {
      tone = button.dataset.tone as Tone;
      document
  ...
  window.addEventListener('pagehide', () => {
    ...
    toggle.setAttribute('aria-pressed', 'false');
    toggle.textContent = 'Enable sound';
    document.querySelector<HTMLElement>('#experience')!.dataset.enabled = 'false';
